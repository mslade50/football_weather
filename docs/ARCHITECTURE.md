...
4. **Degradation records, not silent fallbacks.** Every fallback emits `Degradation{component, reason, severity in info|warn|error, run_id}` into `meta.json.degradations[]`; the site renders banners. Telegram remains bet-focused unless `TELEGRAM_SYSTEM_ALERTS=1` explicitly enables aggregated SYSTEM paging.
...
Copied from `golf_scraping/board/worker/index.js` (`dispatchBoard`, `boardRunActive`, `notifyTelegram`, `ghHeaders`). On each cron fire: (1) write `board/cf_heartbeat.json {ts, cron}` to R2 first; (2) trim in-handler by ET clock (see §9); (3) POST `https://api.github.com/repos/{owner}/football_weather/actions/workflows/pipeline.yml/dispatches` `{ref:'main', inputs:{sport, scope, force}}` using `GH_DISPATCH_TOKEN`; (4) record dispatch failures in Worker logs, with optional Telegram paging only when `TELEGRAM_SYSTEM_ALERTS=1`. GitHub `schedule:` is kept as a backstop.
...
| `board/backtest.json` | weekly bet audit + season backtest (Phase 6): `{meta {...}, postmortem {methodology, latest {window, summary, bets}, season {summary, by_week/by_sport/by_tier/by_book/by_model, bets}}, grid [118 legacy buckets + current/legacy results], stadium_results, stadium_results_legacy, games, alerts_clv}`. Each post-mortem bet is the first actionable alert per game/market/side and preserves that price/edge, its exact same-book close/CLV, result/units, and first/final/historical-model weather; later book repeats stay in the raw CLV audit instead of inflating the bet count. The Backtest tab opens on the weekly post-mortem and keeps the legacy grid as a separate section. |
...
- **Forecast** `https://api.open-meteo.com/v1/forecast` — `models=ncep_nbm_conus,ncep_hrrr_conus,ncep_gfs_seamless,ecmwf_ifs025,ecmwf_aifs025_single`, `hourly=temperature_2m,precipitation,precipitation_probability,wind_speed_10m,wind_gusts_10m,wind_direction_10m`, `wind_speed_unit=mph&temperature_unit=fahrenheit&precipitation_unit=mm`, `timezone=UTC`, `start_hour/end_hour` = kickoff−1h .. kickoff+4h UTC, batched `latitude=a,b,..&longitude=..` ≤50 points/call. International venues: `best_match,ecmwf_ifs025,ecmwf_aifs025_single` since CONUS models return null. ECMWF **AIFS** (data-driven, 0.25°, 15 d) is the `ecmwf_aifs025_single` id (the bare `ecmwf_aifs025` id answers with nulls); it is 6-hourly and Open-Meteo interpolates it to hourly; it has temp / wind / dir / precip but **no gusts and no PoP**.
- **Ensemble** `https://ensemble-api.open-meteo.com/v1/ensemble` — primary request: `models=ecmwf_ifs025,gfs_seamless`, `hourly=wind_speed_10m,wind_gusts_10m,precipitation`. `wind_vol_fc = P90−P10` of pooled member wind over kickoff..+3h; `wind_p10/p50/p90`; `precip_prob_ens` = fraction of members with precip >0.1 mm in window. If a full-member batch receives HTTP 429, only that batch is re-requested with the lighter precomputed `ncep_gefs_ensemble_mean_seamless` model and mean + spread variables. Its wind P10/P90 use mean ± 1.28155 standard deviations (normal approximation; hourly spreads are averaged conservatively over the game window); deterministic/NWS precipitation probability remains in force. Static wind volatility is used only if both calls fail.
...
- **Stitching** (`weather/merge.py`), stamped per row as `source` + `lead_hours`:
  - lead ≤18 h (≤48 h when a synoptic HRRR run covers the window): wind/gust/temp/precip from HRRR; PoP from NBM. `source=hrrr`.
  - 18 h < lead ≤ 48 h: wind mean/temp/PoP/precip mm from NBM; gusts from GFS (NBM lacks gusts). `source=nbm`.
  - 48 h < lead ≤ 7 d (`forecast_blend.medium_range_start_h`, default 168): NBM first; whatever NBM lacks (gusts, nulls) comes from the medium-range blend instead of a single model. `source=nbm`.
  - lead > 7 d: **medium range** = weighted mean of {AIFS, IFS, GFS} with `forecast_blend.medium_range_weights` (default aifs 0.4 / ifs 0.35 / gfs 0.25). Per field, members with a null drop out of that field's mean (AIFS has no gusts/PoP → gusts = IFS+GFS), so a missing member never zeroes a field; direction is the weighted vector mean. `source=medium:aifs+ifs+gfs` (members that covered the window); Degradation(info, `low_confidence`).
  - NWS replaces any null field ≤7 d (`+nws` suffix when it supplied wind); if Open-Meteo fails entirely → NWS-only + Degradation(warn).
  - `model_disagreement = max−min` of deterministic model wind means (AIFS is a member).
- **Climatology shrinkage** (`weather/climatology_blend.py`, applied in `merge.build_forecast` after the window means): `blend(forecast, climo, lead_hours, kind) = w(lead)·forecast + (1−w)·climo`, curves per kind (`wind` — gusts share it — / `temp` / `rain_prob`) from `data/calibration.json` `forecast_blend.weights`: w = 1 for lead ≤ `full_h` (48), linear to `floor` at `floor_h` (168) — or piecewise-linear through fitted `points [[lead_h, w], …]` — held beyond. Defaults until fitted: floors wind 0.45 / temp 0.7 / rain_prob 0.5; `scripts/fit_forecast_blend.py` fits them (previous-runs day-1..7 forecasts vs ERA5 actuals, MAE-optimal shrinkage, pooled over ~40 stadiums, Sep–Dec 2024+2025) and writes `weights` + `fit` stats. Blended: `wind_fg`, `gust_fg`, `temp_fg`, `precip_prob` and the ensemble `wind_p10/p50/p90` (→ `wind_vol_fc`); rain AMOUNT is never blended. `wind_fg`/`temp_fg`/`rain_fg` stay the values impact/signals consume; `wind_fg_raw`, `temp_fg_raw`, `blend_w` (wind weight), `climo_wind`, `climo_temp` are emitted beside them. Base rate = `data/climatology.csv` cell for stadium × ISO week × 6-h solar-time bin (`round(lon/15)` h offset; bins 00-06/06-12/12-18/18-24) of kickoff+1h: mean/P10/P50/P90 wind and temp, gust mean/P90, hourly ≥1 mm rain frequency; built by `stadiums/climatology.py` from ONE hourly ERA5 request per stadium (2015-01-01..2024-12-31, Aug–Jan cells only, 1.5 s throttle, raw cached under `--cache-dir`). The same csv keeps the legacy per-stadium summary row (`avg_wind_sep..jan`, `avg_temp_f`; written last per stadium so last-row-wins readers keep working). The cell is found by `stadium_id` or the nearest stadium (≤0.3°) to the Open-Meteo coordinates; no cell → `blend_w=1` + Degradation(info).
...
- **CLV** (`model/clv.py`): at first run after kickoff, freeze `closings` = last `odds_history` row before `kickoff_utc` per key; `clv_pts = signed(closing_line − alert.first_line)` in the alerted side's favor; written to `alerts.clv_pts`. Tuesday's post-mortem combines CLV with results and weather verification; the CLV-only digest remains available manually.
...
- `backtest.yml`: schedules `'17 13 * * 0'` (Sun 09:17 EDT / 08:17 EST, CFB after Saturday) and `'17 13 * * 2'` (Tue 09:17 EDT / 08:17 EST, NFL after Monday night) + dispatch → `python -m pipeline.backtest` → R2 `board/backtest.json` + `data/backtest/*.parquet`; each scheduled run sends the matching league's post-mortem via `pipeline.alerts --digest postmortem --sport …`.
...
Table view: all GameCard columns, sortable, filters (sport, week, signal, min edge, book, kickoff window, domes); default view under 700 px. Alerts tab: `alerts_feed.json` with filters and CLV. Backtest tab: weekly post-mortem (default), season/week rollups, bucket grid, stadium results, and matched games. Status tab: last 20 runs, stage timings, counts vs baseline, degradations, unresolved names.
...
## 14. Secrets
GitHub repo secrets: `CLOUDFLARE_API_TOKEN` (Workers Scripts:Edit, R2:Edit, D1:Edit), `CF_ACCOUNT_ID` (= ba4875f01f2bc46dd48e1e26d2ec9080, exported as `CLOUDFLARE_ACCOUNT_ID`), `TELEGRAM_BOT_TOKEN`, `TELEGRAM_CHAT_ID` (+ optional `TELEGRAM_CHAT_ID_NFL`, `TELEGRAM_CHAT_ID_CFB`), optional weekly-report SMTP (`POSTMORTEM_EMAIL_TO`, `SMTP_HOST`, `SMTP_PORT`, `SMTP_USERNAME`, `SMTP_PASSWORD`, `SMTP_FROM`, `SMTP_USE_SSL`, `SMTP_STARTTLS`), `CFBD_API_KEY`, `PROPHETX_API_KEY` or `PROPHETX_ACCESS_KEY`+`PROPHETX_SECRET_KEY`, optional `ODDS_API_KEY`, optional `R2_ACCESS_KEY_ID`/`R2_SECRET_ACCESS_KEY` (boto3 path), optional `NOVIG_CLIENT_ID`/`NOVIG_CLIENT_SECRET` (NBX fallback). Optional variable `TELEGRAM_SYSTEM_ALERTS=1` enables aggregated operations paging; default is `0`. Derived env: `PROPHETX_ENABLED`, `BOOK_*_ENABLED`.
