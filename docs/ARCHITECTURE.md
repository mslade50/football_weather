...
Families and keys:
1. **EDGE** `edge|{season}|{week}|{game_id}|{market}|{side}|{book}|{model_version}` — fires on tier ∈ {edge, strong} (§7.3) and weather-driven condition. One send per key. Message:
```
<b>🌬 NFL Wk 3 · SEA @ NE · Sun 1:00p ET</b>
Gillette · wind 18 mph SE (gust 26 · vol 6 · cross 15) · 41°F · rain 20% / 0.8 mm
Impact −6.5% (wind 6.5) · conf 0.72 · fair total 34.6 (ref pinnacle, 6 books)
<b>UNDER 38.0 −110 @ BetOnline</b> · edge 3.4 pts / +4.1% · open 38.0
Best: Under 38.5 −108 Betcris · FD 38.0 · Kalshi 37.5 (52¢) · Novig 38.0
<a href="https://football-board.<acct>.workers.dev/#sport=nfl&week=3&game=nfl:2026:3:sea@ne">board</a>
```
2. **MOVE** on alerted keys only: `move|{edge_key}|{bucket}`, bucket = `floor(|line_now − last_sent_line| / step)`, step 1.0 total / 0.5 spread; direction 'toward fair' / 'away from fair'; ≤1 per key per 2 h. **EDGE GONE**: `gone|{edge_key}` when edge_pts < 0.5 → status=closed. **FORECAST MOVE**: `wx|{edge_key}|{bucket}` when fair line moved ≥1.0 pt due to weather; message shows old→new wind/rain.
...
Rate/quiet: max 25 messages per run, overflow → one digest; quiet hours 23:00–07:00 ET queue non-strong alerts in `telegram_state.json` and flush as 07:00 digest; `strong` tier and kickoff <3 h bypass. Optional routing `TELEGRAM_CHAT_ID_NFL` / `TELEGRAM_CHAT_ID_CFB` fallback to `TELEGRAM_CHAT_ID`. `--no-alerts` and `--dry-run` print instead of sending.
