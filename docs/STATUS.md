...
- Backtest / calibration / CLV grids for **2026** are structurally complete but empty until games settle (first kickoff 2026-08-29); the v2 promotion gate cannot fire before ≥4 distinct weeks. `calibrate.py` needs `backtest/games.parquet` rows carrying `total_open/total_close`. The **2024 + 2025** columns are filled by the git-archive replay (`pipeline/backtest_git.py`, ARCH §7.6, `docs/HISTORICAL_BACKTEST_SPEC.md` §7): 1,782 games, 1,614 graded at the close, 521 alerted, v1 replay match 0.9973. Its ERA5 cache (`data/backtest/era5/`, ~670 MB) is local-only; CI restores the reduced `windows.parquet` from R2 (SETUP.md §10).
